...
import { unlockReqStatus } from '../../../Services/Admin/UnlockRequest';
...
import { getUnlockRequestetails } from '../../../Services/Admin/UnlockRequest';
...
const UnlockRequestDetails = () => {
  ...
  const [rejectRequestReasonModal, setRejectRequestReasonModal] = useState(false);
  ...
  // Mutation for accepting and rejecting request
  const unlockRequestStatus = useMutation({
    mutationFn: (formData) => unlockReqStatus(id, formData),
    onSuccess: () => {
      showToast('Unlock request status updated successfully!', 'success');
      queryClient.invalidateQueries(['getUnlockRequestetails', id]);
    },
    onError: (error) => {
      console.error('Error unlocking request status:', error);
  ...
  const handleRequestRejection = (values) => {
    ...
    unlockRequestStatus.mutate(newValues);
  }
        ...
        {isLoading ? Array.from({ length: 8 }).map((_, i) => (
          ...
          <div className="row ">
            <div className="col-12 col-lg-10 col-xl-9 col-xxl-7 mb-4">
              ...
              {unlockRequestDetail?.status === 'pending' && (
      ...
      >
        ...
        <div className="px-sm-5">
          <Formik
            ...
            onSubmit={(values) => handleRequestRejection(values)}
          >
