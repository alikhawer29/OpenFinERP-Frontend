...
const EditWalkInCustomer = () => {
  usePageTitle('Walk In Customer - Edit');
  ...
  const [selectedVATState, setSelectedVATState] = useState(null);
  ...
  useEffect(() => {
    if (walkInCustomer?.vat_country?.id && !isNullOrEmpty(nationalities)) {
      setSelectedVATCountry(walkInCustomer.vat_country.id);
    }
  }, [nationalities, walkInCustomer]);

  useEffect(() => {
    if (!isNullOrEmpty(walkInCustomer)) {
      setSelectedNationality(walkInCustomer?.nationality?.id);
    }
  }, [walkInCustomer]);

  useEffect(() => {
    if (!isNullOrEmpty(states) && !loadingStates) {
      setSelectedVATState(walkInCustomer?.vat_state?.id);
    }
  }, [selectedVATCountry, states]);

  const editWalkInCustomerMutation = useMutation({
    mutationFn: (formData) => editWalkInCustomer(id, formData),
    onSuccess: () => {
      showToast('Walk-in Customer Updated!', 'success');
      setTimeout(() => {
        navigate(-1);
      }, 300);
    },
  ...
  const handleSubmit = (values) => {
    ...
    editWalkInCustomerMutation.mutate(formData);
  };
        ...
        <div className="row">
          <div className="col-12 col-lg-10 col-xl-9 col-xxl-7">
            <Formik
              initialValues={{
                ...walkInCustomer,
                mobile_number: walkInCustomer?.mobile_number_full,
                fax_number: walkInCustomer?.fax_number_full,
                telephone_number: walkInCustomer?.telephone_number_full,
                nationality: nationalities?.find(
                  (x) => x.value === walkInCustomer?.nationality?.id
                )?.value,
                vat_country: nationalities?.find(
                  (x) => x.value === walkInCustomer?.vat_country?.id
                )?.value,
                vat_state: states?.find(
                  (x) => x.value === walkInCustomer?.vat_state?.id
                )?.value,
                id_type: walkInCustomer?.id_type?.id,
              }}
                  ...
                  <div className="row mb-4">
                    ...
                    <div className="col-12 col-sm-6 mb-45">
                      <SearchableSelect
                        ...
                        onChange={(v) => {
                          ...
                          setFieldValue('vat_state', undefined);
                          setSelectedVATCountry(v.value);
                          setSelectedVATState(undefined);
                        }}
                    ...
                    <div className="col-12 col-sm-6 mb-45">
                      <SearchableSelect
                        ...
                        value={selectedVATState ? selectedVATState : ''}
                        onChange={(v) => {
                          setFieldValue('vat_state', v.value);
                          setSelectedVATState(v.value);
                        }}
...
export default EditWalkInCustomer;
