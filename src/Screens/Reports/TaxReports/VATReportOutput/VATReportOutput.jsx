...
const VATReportOutput = ({
  ...
  useEffect(() => {
    if (selectedItem) {
      ...
      setSelectedVATCountry(selectedItem?.vat_country);
    }
  }, [selectedItem]);
                ...
                <tbody>
                  ...
                  {tableData?.map((item, index) => (
                    ...
                    >
                      ...
                      {urlData?.vat_type !== 'zero_rate' && (
                        <>
                          <td>{item.base_taxable_amount || '0.00'}</td>
                          {item.reason && <td>{item.reason}</td>}
                          {item.base_vat_amount && (
                            <td>{item.base_vat_amount || '0.00'}</td>
                          )}
      ...
      <CustomModal show={showEditModal} close={() => setShowEditModal(false)}>
        ...
        <div className="px-sm-5">
          <Formik
            ...
            initialValues={{
              vat_trn:
                editDetails?.data?.current_vat_trn || selectedItem?.trn || '',
              vat_country:
                editDetails?.data?.current_vat_country ||
                selectedItem?.vat_country ||
                '',
              vat_state:
                editDetails?.data?.current_vat_state ||
                selectedItem?.vat_state ||
                '',
              vat_exempted:
                editDetails?.data?.current_vat_exempted ||
                selectedItem?.vat_exempted ||
                false,
            }}
...
export default withModal(withFilters(VATReportOutput));
