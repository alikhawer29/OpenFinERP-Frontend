...
const EditTmnCurrencyDeal = ({
  ...
  const handleSubmit = async () => {
    ...
    if (!(tmnCurrencyDealData?.id || tmnCurrencyDeal?.id)) {
      showToast('Transaction ID is missing. Please refresh.', 'error');
      setIsSubmitting(false);
      return;
    }
    ...
    // Build payload matching the New page's structure:
    // Use ...formValues spread (safe now that SC data is sanitized - no nested objects to leak).
    // The original rate_type error was caused by SC nested objects being flattened by buildFormData,
    // NOT by having extra form fields. Also add 'ledger' which the update endpoint requires.
    let payload = {
    ...
    if (
      ...
      allocations.forEach((allocation, index) => {
        Object.entries(allocation).forEach(([key, value]) => {
          // Skip 'id' and skip nested objects/arrays from API response
          // (e.g., account_details, bank objects) to prevent buildFormData
          // from flattening them into top-level keys
          if (key === 'id') return;
    ...
    // Handle Special Commission - exactly matching New page's code
  ...
  useEffect(() => {
    if (tmnCurrencyDeal) {
      ...
      if (tmnCurrencyDeal?.special_commission) {
        const rawSC = tmnCurrencyDeal.special_commission;
        // Only keep the fields needed for display and submission.
        // Strip nested objects (currency, account, etc.) and API metadata
        // to prevent buildFormData from flattening them into top-level keys
        // (e.g., currency.rate_type = 'Multiply' would overwrite form's rate_type = '/').
        const scData = {
          commission_type: rawSC.commission_type,
          commission: rawSC.commission,
          total_commission: rawSC.total_commission,
          currency_id: rawSC.currency?.id || rawSC.currency_id || rawSC.amount_type?.id,
          amount: rawSC.amount,
          description: rawSC.description,
          distributions: (rawSC.commission_distribution || rawSC.distributions || []).map((dist) => {
            // Keep all primitive fields from each distribution, skip nested objects
            const cleanDist = {};
            ...
            // Remove API metadata fields that shouldn't be sent
            delete cleanDist.id;
            delete cleanDist.created_at;
            delete cleanDist.updated_at;
            delete cleanDist.deleted_at;
            return cleanDist;
          }),
        };
  ...
  const getInitialValues = () => {
    ...
    const initialValues = {
      ...
      commission_currency_id: tmnCurrencyDeal?.commission_currency_id || tmnCurrencyDeal?.fc_currency_id || '',
      ...
      <div className="d-card">
        <Formik
          ...
          validate={(values) => {
            ...
            if (!values.date) errors.date = 'Voucher Date is required';
            ...
            if (values.rate) {
              ...
              if (!isNaN(rateValue)) {
                ...
                if (officialRate) {
                  ...
                  if (!isNaN(minRange) && !isNaN(maxRange) && (rateValue < minRange || rateValue > maxRange)) {
          ...
          {({
            ...
            useEffect(() => {
              ...
              if (values.vat_terms_id && vatTermsOptions) {
                ...
                if (selectedVatTerm) {
                  if (selectedVatTerm.percentage) {
                    setFieldValue('vat_percentage', selectedVatTerm.percentage);
                  }
                  // Also set vat_terms (title) - the API may not return it directly
                  if (selectedVatTerm.title && !values.vat_terms) {
                    setFieldValue('vat_terms', selectedVatTerm.title);
                  }
                }
              }
            ...
            useEffect(() => {
              // Don't clear during initial data load - addedSpecialCommissionValues
              // is set asynchronously and may not be ready yet
              if (isLoadingInitialDataRef.current) return;
                    ...
                    <div className="row">
                      ...
                      <div className="col-12 col-sm-6 mb-45">
                        <CombinedInputs
                          ...
                          onChange1={(selected) => {
                            ...
                            setFieldValue('base_rate_currency_id', selected.value);
                            // When FC currency changes, clear existing rate so that
                            // the dual-currency-rate API can repopulate it for the new pair.
                            setFieldValue('rate', '');
                            setShowRateError(false);
                            if (!addedSpecialCommissionValues) {
                              setFieldValue(
                      ...
                      <div className="col-12 col-sm-6 mb-45">
                        <CombinedInputs
                          ...
                          value2={formatRateValue(values.base_rates)}
      ...
      <CustomModal
        ...
        action={() => {
          ...
          navigate('/transactions/remittance-rate-of-exchange', {
            state: { currencyToSelect: selectedCurrency, date },
          });
        }}
      />
